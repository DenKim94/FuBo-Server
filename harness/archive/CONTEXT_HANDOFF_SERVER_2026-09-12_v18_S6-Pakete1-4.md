...
>   `CONTEXT_HANDOFF_SERVER_2026-09-06_v17_S5-gebaut.md`, die letzte Langfassung mit allen
...
**S0 bis S4 sind abgeschlossen, S5 ist vollständig gebaut, S6 steht bis Paket 4.** Der letzte
Testlauf (06.09.2026) lief mit **385 Fällen in 29 Klassen**; ein Fall schlug fehl, die Ursache ist
behoben. **Zwei Läufe stehen damit aus – der Bestätigungslauf zu S5 und der erste zu S6.** Beide
brauchen Docker (Testcontainers, `postgres:17`) und laufen ausschliesslich lokal. **Vor dem grünen
Lauf wird nichts committet.**

**Der Vertrag steht unverändert bei 34 Endpunkten** – S6 bringt drei neue, aber sie werden erst in
**Paket 6** eingetragen; Code ohne Vertrag ist der Zwischenzustand, den diese Reihenfolge bewusst
in Kauf nimmt. Das Datenmodell bleibt bei 18 Tabellen in drei Schemas (`V001`–`V011`): **S6 kommt
ohne Migration aus**, `spieltag.ergebnis` steht seit `V006`, die drei Zähler seit `V011`.

### Was S6 bis Paket 4 gebracht hat

Die Ergebniserfassung (`POST /ergebnis/erfassen`, offen für jeden Angemeldeten), die
Admin-Korrektur (`POST /admin/ergebnis/korrigieren`) und die Bilanzrechnung samt eigenem Lesepfad
(`GET /bilanz/lesen`). **Drei neue Fehlercodes** (`ERGEBNIS_VORHANDEN`,
`TERMIN_NICHT_ABGESCHLOSSEN`, `KEINE_EINTEILUNG`) und **zwei neue Audit-Aktionen**
(`ERGEBNIS_ERFASST`, `ERGEBNIS_KORRIGIERT`). `ergebnis.sieger` ist die zweite `CHAR(1)`-Spalte des
Schemas und ab jetzt als `Character` gemappt.

**Zwei Entscheidungen des Haupt-Entwicklers vom 12.09.2026**, beide als Rückfrage vor der ersten
Zeile Code gestellt, weil die Anleitung sich an diesen Stellen widersprach:

1. **Die eigene Bilanz bekommt einen eigenen Endpunkt.** `S6_UMSETZUNG.md`, 4.5 sagt „auch die
   registrierten Spieler", 5.3 und 6 sagen „vorerst nur unter `/admin/`". Additiv ging es nicht:
   Die API kennt bis heute **keinen** Endpunkt, über den ein `USER` sein eigenes Profil liest.
   Also `GET /api/v1/bilanz/lesen`, **ohne Id im Pfad** – mit einer Id wäre es ein Endpunkt
   „fremde Bilanz lesen". Der Vertrag wächst damit in Paket 6 auf **37** Endpunkte, nicht 36.
2. **Die Löschfrist des Audit-Logs geht von 90 auf 30 Tage** (`0.6`, Ergänzung). Eine harte
   Obergrenze der Zeilenzahl kam **nicht** dazu: Sie würfe in einem Ansturm genau die Einträge
   weg, die ihn belegen. `fubo.reset.aufbewahrung-tage` stand schon auf 30 und ist damit nicht
   mehr die kürzere Frist – länger als das Protokoll darf sie nie werden.

### Was an S6 noch fehlt

**Pakete 5 bis 8:** der Lesepfad in der Einzelansicht (`TerminDetails.ergebnis`), der Vertrag samt
Bruno-Ordner, die Tests (`ErgebnisControllerTests`, Zeitstreifen **600 Tage/21:30**) und die
Verifikation. **Die Testanzahl steht deshalb weiter bei 385** – Paket 4 hat keine Testklasse
angelegt.

### Der eine Fehlschlag aus S5 – und warum er lehrreich ist
...
die Liste zu S5 (`S5_UMSETZUNG.md`, 13.1) und die drei Punkte zur Gastverwaltung aus 6.4. Dazu
kommt ab jetzt die Liste zu S6 (`S6_UMSETZUNG.md`, 8.1). Alle brauchen eine laufende Anwendung und
werden **nach dem grünen Testlauf in einem Zug** abgearbeitet.
...
| S6 | **Ergebnis und Bilanz** (der Name „Ergebnis & Audit API" versprach einen Leseendpunkt fürs Protokoll, den keine Anforderung verlangt): „erster Eintrag gilt", Admin-Korrektur, Bilanz-Zähler | **Pakete 1–4 gebaut (12.09.2026)**, ungetestet; Pakete 5–8 offen. Schrittsumme **11,0 h** plus rund 1,0 h für die beiden Entscheidungen vom 12.09. | 8 |
...
**Fachbereich `ergebnis` (S6, Pakete 1 bis 4, 12.09.2026):**

```
domain/spieltag/      Sieger              Aufzaehlungstyp A/B/U; kennung() traegt das Zeichen
                      Ergebnis            JPA-Entity; sieger als Character, @Version fuer die
                                          Korrektur, erfasst_am aus dem Spalten-Default
domain/profil/        Bilanzstand         drei Zaehler als Wertobjekt, mit Konstante LEER
repository/spieltag/  ErgebnisRepository  JPA + Custom: findByTerminId ueber die Entity,
                      (+Custom/+Impl)     einfuegen() mit ON CONFLICT DO NOTHING RETURNING id
repository/profil/    BilanzRepository    eine UPDATE-Anweisung ohne Entity, dazu ein Lesezugriff
service/ergebnis/     ErgebnisService     Pruefreihenfolge, Transaktionsgrenze, Audit
service/profil/       BilanzService       Neuberechnung + Verwerfen des Zwischenspeichers
controller/ergebnis/  ErgebnisController          POST /ergebnis/erfassen, 201
controller/admin/     ErgebnisVerwaltungController POST /admin/ergebnis/korrigieren, 204
controller/profil/    BilanzController            GET  /bilanz/lesen, 200
dto/spieltag/         Ergebnis, ErgebnisErfassenRequest, ErgebnisKorrigierenRequest
dto/profil/           Bilanz
```

**Der DTO `Ergebnis` und die Entity `Ergebnis` heissen gleich** – der Vertrag führt das Schema so,
die Entity heisst nach ihrer Tabelle. Sie treffen sich ausschliesslich in `Ergebnis#von`, wo der
Quelltyp deshalb voll qualifiziert steht; jede andere Klasse importiert genau einen von beiden.

**Der Zaehler `BilanzRepository#neuBerechnen` ist die Stelle, an der S6 falsch werden kann, ohne
dass es auffaellt.** Zwei Dinge daran sind nicht austauschbar: Die **innere** Unterabfrage grenzt
auf die Beteiligten dieses einen Termins ein, die **äussere** zählt für sie über *alle* ihre
Termine – wer nur über den einen zählt, löscht jedem Spieler die Historie. Und `version` steigt
mit, sonst schreibt eine gleichzeitig geladene `Spieler`-Entity die alte Bilanz still zurück.

**`ErgebnisService#korrigieren` benutzt `saveAndFlush`, nicht `save`.** Die Bilanzrechnung liest
`spieltag.ergebnis` **per SQL** und sieht nur, was in der Datenbank steht; ohne Flush rechnete sie
gegen den alten Ausgang, und die Zahlen blieben dabei plausibel. Nebenbei fällt der Sperrkonflikt
damit an der Aufrufstelle an statt als `UnexpectedRollbackException` beim Commit. **Der
Eintragspfad hat das Problem nicht** – er schreibt nativ.
...
1. **`./mvnw clean verify` laufen lassen.** Erwartet werden **385 Fälle in 29 Klassen**, grün – die
   Zahl steht unverändert, weil S6 bis Paket 4 keine Testklasse angelegt hat. Der Lauf deckt damit
   zweierlei ab: den ausstehenden Bestätigungslauf zu S5 (der Fehlschlag
   `verschiebenInDieZukunftSetztDieFixierungZurueck` ist behoben) **und** die Frage, ob der neue
   Code den Kontext überhaupt hochbringt. **Der wahrscheinlichste Bruch ist genau dort:**
   `ddl-auto=validate` prüft `ergebnis.sieger` gegen die Entity, und ein falsch gemapptes `CHAR(1)`
   zeigt sich als Kaskade von `UnsatisfiedDependencyException`, bei der **nur die erste Logzeile**
   die Ursache nennt. **Scheitert etwas, zuerst die Surefire-Berichte lesen**, nicht die
   Maven-Zusammenfassung – Vorgehen in 6.4.
2. **Erst danach committen.** Themenschnitt für S5 in `S5_UMSETZUNG.md`, 13.2; für S6 bis Paket 4:
   Audit-Frist (Konfiguration und Doku), Ergebnis erfassen, Korrektur, Bilanz, Doku. **Der Vertrag
   fehlt in dieser Liste** – er kommt mit Paket 6 und geht dann seinen Commits voraus.
3. **S6 fortsetzen, Pakete 5 bis 8.** Der Lesepfad (`TerminDetails.ergebnis`, nullbar), der Vertrag
   **auf 37 Endpunkte** samt Bruno-Ordner `ergebnis/`, die Tests und die Verifikation.
   - **Die Testklasse bekommt den Zeitstreifen 600 Tage/21:30.** Vergeben sind 40/18:15, 120/19:45,
     200/17:30, 300/16:05 und 500/20:15; **beide Achsen zählen.**
   - **Der Aufbau je Fall ist der eigentliche Aufwand:** Termin in der Vergangenheit anlegen,
     Zusagen setzen, über `POST /teams/generieren` einteilen, **danach** per SQL auf
     `ABGESCHLOSSEN` setzen (mit `version = version + 1`). **Umgekehrt geht es nicht** – für einen
     abgeschlossenen Termin lässt sich nicht mehr generieren.
   - **Der wertvollste Fall ist der mit zwei Terminen und einer Korrektur am ersten.** Er ist der
     einzige, der „neu berechnen" von „fortschreiben" unterscheidet, und zugleich der einzige, der
     bemerkt, wenn die Neuberechnung nur über den einen Termin zählt.
   - **Profiländerung und Ergebniserfassung für denselben Spieler gehören nicht in dieselbe
     Test-Transaktion** – die Bilanzrechnung erhöht `spieler.version` nativ, und eine im selben
     Persistence-Context geladene `Spieler`-Entity läuft danach in einen Sperrkonflikt, den es im
     Betrieb nicht gäbe.
4. **Danach alle Handprüfungen in einem Zug** – entschieden am 06.09.2026, weil eine halbe
   Generierung nichts zeigt, was sich prüfen liesse. Es sind vier Listen:
   - **S6, `S6_UMSETZUNG.md` 8.1** – allen voran die SQL-Gegenprobe, die die Bilanz gegen die
     Ergebnisse zählt, und „zweimal korrigieren": `korrigiert_am` wandert, zwei Audit-Einträge.
...
5. **Client-Track informieren.** Drei brechende Vertragsänderungen aus früheren Meilensteinen
   (4.1), **alles Neue aus S5** (4.3) und, sobald Paket 6 steht, das Neue aus S6: dass `ergebnis`
   in der Einzelansicht `null` sein darf und das der Normalzustand ist; dass **`ERGEBNIS_VORHANDEN`
   kein Bedienfehler ist** (nach dem Abpfiff tippen mehrere gleichzeitig – der Zweite soll „hat
   schon jemand gemacht" sehen und danach das erfasste Ergebnis); dass die Korrektur die `version`
   braucht und `DATEN_VERALTET` „neu laden" heisst, nicht „Eingabe falsch"; dass der Haken
   „deutlich" bei Unentschieden auszublenden ist; und dass es **keinen Weg gibt, ein Ergebnis zu
   löschen** – das gehört in die Bestätigungsabfrage.
